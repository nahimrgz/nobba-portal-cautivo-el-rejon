...
var chilliController = { interval:30 , host:"1.0.0.1" , port:false , ident:'00' , ssl:false , uamService: false , uamSecret:"", client:"0.0.0.0", isL2: false};
...
chilliController.urlRoot = function () {
    var portalProxyOn = chilliController.host == "login.altaitechnologies.com";
    var protocol = ( chilliController.ssl && portalProxyOn) ? "https" : "http" ;
    var urlRoot = null;

    if(portalProxyOn && !chilliController.isL2){
        urlRoot = protocol + "://" + chilliController.host + (chilliController.port ? ":" + chilliController.port.toString() : "") + "/UAM/json/";
    }else{
        urlRoot = protocol + "://" + chilliController.host + (chilliController.port ? ":" + chilliController.port.toString() : "") + "/json/";
    }
    return urlRoot;
};
...
chilliController.onUpdate = function ( cmd ) {
	console.log('>> Default onUpdate handler. <<\n>> You should write your own. <<\n>> cmd = ' + cmd + ' <<' );
};
...
chilliController.logon = function ( username , password )  {
    ...
    if(chilliController.isL2){
        console.log ('chilliController.l2logon( "' + username + '" , "' + password + '" )' );
        console.log ('chilliController.l2logon: go to AP Local Portal Page' );

        var params = chilliController.search + "&uname=" + escape(username) + "&pword=" + escape(password);
        var urlRoot = "https://" + chilliController.host + "/json/www/package/index.html" + params;
        ...
        chilliJSON.get( urlRoot ) ;
    }
    else{
    	console.log ( 'chilliController.logon( "' + username + '" , "' + password + '" )' );

    	chilliController.temp = { 'username': username , 'password': password };
    	chilliController.command = 'logon';

    	console.log ('chilliController.logon: asking for a new challenge' );
    	chilliJSON.onError        = chilliController.onError    ;
...
chilliController.logon2 = function ( username , response )  {
	...
	console.log ( 'chilliController.logon2( "' + username + '" , "' + response + ' " )' );

	chilliController.temp = { 'username': username , 'response': response };
	chilliController.command = 'logon2';

	console.log ('chilliController.logon2: asking for a new challenge ' );
	chilliJSON.onError        = chilliController.onError    ;
	chilliJSON.onJSONReady    = chilliController.logonStep2 ;
	chilliController.clientState = chilliController.AUTH_PENDING ; 
	chilliJSON.get( chilliController.urlRoot() + 'status'  ) ;
...
chilliController.logonStep2 = function ( resp ) {
	...
	console.log('Entering logonStep 2');

	if ( typeof (resp.challenge) != 'string' ) {
		console.log('logonStep2: cannot find a challenge. Aborting.');
		return chilliController.onError('Cannot get challenge');
	}

	if ( resp.clientSate === chilliController.stateCodes.AUTH ) {
		console.log('logonStep2: Already connected. Aborting.');
	...
	console.log ('chilliController.logonStep2: Got challenge = ' + challenge );

	if ( chilliController.uamService ) { /* MD5 CHAP will be calculated by uamService */

        var url = null;
		if (chilliController.uamService == 'cleartext') {
            console.log('chilliController.logonStep2: Logon using ap uamService');
            // Build command URL
            url = chilliController.urlRoot() + 'logon?username=' + escape(username) + '&password=' + escape(password) + '&challenge=' + challenge;

			// Make uamService request
			chilliJSON.onError     = chilliController.onError;
			chilliJSON.onJSONReady = chilliController.processReply;
        } else {
            console.log('chilliController.logonStep2: Logon using uamService (external MD5 CHAP)');
            ...
            // Build command URL
            url = chilliController.uamService + c + 'username=' + escape(username) + '&password=' + escape(password) + '&challenge=' + challenge;
		...
		if (!response || response == '') {
			...
			if (!chilliController.uamSecret || chilliController.uamSecret == '') {
				/* Case: empty uamsecret */
				console.log ( 'chilliController.logonStep2: Case empty chilliController.uamSecret' );
				response = myMD5.chap ( chilliController.ident , password , challenge );
				console.log ( 'chilliController.logonStep2: Calculating CHAP-Password = ' + response );
			}
			else {
				/* Case: uamsecret exists */
				var new_challenge = myMD5.chal_uamsecret ( challenge, chilliController.uamSecret );
				console.log ( 'chilliController.logonStep2: Case chilliController.uamSecret exists = ' + chilliController.uamSecret );
				console.log ( 'chilliController.logonStep2: Calculating new CHAP-Challenge with uamSecret = ' + new_challenge );
				response = myMD5.chap ( chilliController.ident , password , new_challenge );
				console.log ( 'chilliController.logonStep2: Calculating CHAP-Password = ' + response );
			}
		}
...
chilliController.logonStep3 = function ( resp ) {
	console.log('Entering logonStep 3');
...
chilliController.processReply = function ( resp ) {
	...
	console.log('chilliController.processReply');
	...
	console.log ('chilliController.processReply: Calling onUpdate. clienState = ' + chilliController.clientState);
...
chilliJSON.expired   = function () {
	...
	if ( chilliJSON.node.text ) {
		console.log ('chilliJSON: reply content \n' + chilliJSON.node.text );
	}
	else {
		console.log ('chilliJSON: request timed out (or reply is not valid JS)');
	}
...
chilliJSON.reply = function  ( raw ) {
	...
	if ( chilliJSON.timestamp ) {
		console.log ( 'chilliJSON: JSON reply received in ' + ( end - chilliJSON.timestamp ) + ' ms\n' + dumpObject(raw) );
	}
    ...
    console.log('chilliJSON.reply: call fn ptr chilliJSON.onJSONReady');
...
chilliJSON.get = function ( gUrl ) {
	...
	if ( typeof(gUrl) == "string" ) {
		...
		console.log ( "chilliJSON:error:Incorrect url passed to chilliJSON.get():" + gUrl );
		chilliJSON.onError ( "Incorrect url passed to chilliJSON.get() " );
		return ;
	}

	if ( chilliJSON.timer ) {
		console.log('logon:   There is already a request running. Return without launching a new request.');
		return ;
	}
    ...
    if(chilliController.isL2){
        var requestUrl = chilliJSON.url + c + 'ip=' + chilliController.client + '&callback=chilliJSON.reply' + '&'+Math.random();
	...
	scriptElement.src = chilliJSON.url + c + 'ip=' + chilliController.client + '&callback=chilliJSON.reply' ;
	...
	console.log ('chilliJSON: getting ' + chilliJSON.url + ' . Waiting for reply ...');
...
chilliClock.onTick = function () {
	console.log ("You should define your own onTick() handler on this clock object. Clock value = " + this.value );
};
...
function ChilliMD5() {
	...
	// Support uamsecret
	this.chal_uamsecret = function ( hex_chal, str_uamsecret ) {
		
		//  Convert everything to hex encoded strings
		var hex_uamsecret = str2hex ( str_uamsecret );
		
		// concatenate hex encoded strings (challenge + uamsecret)
		var hex = hex_chal + hex_uamsecret;
